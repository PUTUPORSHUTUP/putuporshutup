...
BEGIN
  ...
  WITH registration AS (
    ...
    SELECT challenge_id, id, 100, 'joined'
    FROM unnest(player_ids) AS id
    RETURNING user_id
  )
  UPDATE profiles p
  SET wallet_balance = wallet_balance - 100, last_used = NOW()
  FROM registration r
  ...
  WITH prizes AS (
    ...
    AND (mr.placement = 1 OR (mr.placement = 2 AND players_count >= 4) OR (mr.placement = 3 AND players_count >= 6))
  )
  UPDATE profiles p
  ...
  RETURN json_build_object(
    ...
    'paid_rows', COALESCE(players_count, 0)
  );
