-- Fix remaining security issues found by the linter
...
-- Add RLS policies for auth_diagnostics table
CREATE POLICY "Service can manage auth diagnostics"
  ON public.auth_diagnostics FOR ALL
  USING (true);

CREATE POLICY "Admins can view auth diagnostics"
  ON public.auth_diagnostics FOR SELECT
  USING (is_user_admin());
