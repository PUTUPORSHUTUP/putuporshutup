-- Soft Launch Infrastructure - Final Version
...
BEGIN
  ...
  -- 3. Process refunds for cancelled matches (skip if wallet_transactions doesn't exist)
...
BEGIN
  ...
  -- Add them to market queue immediately if table exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'market_queue') THEN
...
DROP VIEW IF EXISTS public.soft_launch_metrics;

-- Create soft launch monitoring view
CREATE VIEW public.soft_launch_metrics AS
SELECT 
  ...
  (SELECT COUNT(*) FROM market_events WHERE created_at > now() - interval '1 hour' AND error_message IS NULL) as successful_events_1h,
