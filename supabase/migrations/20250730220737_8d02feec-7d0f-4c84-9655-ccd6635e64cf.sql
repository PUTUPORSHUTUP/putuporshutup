-- Create an epic Sunday Showdown tournament for 12 noon tomorrow (fixed version)
DO $$
DECLARE
    tournament_id UUID;
...
BEGIN
    ...
    -- Create announcement entry for all users to see
    INSERT INTO match_notifications (
        ...
        'The ultimate weekend warrior battle is live! $1000 prize pool, 16 players, single elimination. Registration open now - match starts tomorrow at 12 noon sharp! Entry fee: $25',
        tournament_id::text,
        NOW(),
        false
    );
    
    -- Log the automation action for tracking
