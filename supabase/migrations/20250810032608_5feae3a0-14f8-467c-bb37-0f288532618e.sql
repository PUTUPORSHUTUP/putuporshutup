...
FOR SELECT USING (
  ...
  match_id IN (
    SELECT mr.match_id FROM match_results mr WHERE mr.player_id = auth.uid()
  ) OR
  is_user_admin()
);
...
-- Create secure role checking for moderators
...
-- Update OTP cleanup to be more aggressive
...
AS $function$
BEGIN
  -- Only allow admins to log admin actions
