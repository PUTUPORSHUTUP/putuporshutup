-- Update atomic_market_cycle to use an existing test user as creator instead of system user
...
BEGIN
  ...
  BEGIN
    ...
    -- 2. Get available test users (with lock to prevent race conditions)
    ...
    FROM (
      ...
      LIMIT min_players + 1  -- Get one extra for creator
    ...
    -- Use first user as creator, rest as participants
    creator_user_id := user_ids[1];
    user_ids := user_ids[2:array_length(user_ids, 1)];  -- Remove creator from participants
    participants_count := participants_count - 1;
    
    -- 3. Create challenge using an existing test user as creator
    ...
    -- 5. Join other players atomically
    ...
    -- 6. Determine if we should crash (for testing)
    should_crash := NOT force_no_crash AND random() < crash_rate;
    
    IF should_crash THEN
      -- Simulate crash scenario - refund all players including creator
      ...
      RETURN jsonb_build_object(
        ...
        'participants', participants_count + 1,
        'message', 'Challenge crashed - all players refunded'
      );
    END IF;
    
    -- 7. Generate results (create stats for creator and participants)
    INSERT INTO challenge_stats (
      ...
      1, -- creator gets placement 1 (could be randomized)
      true
    );
    ...
    FOR i IN 1..participants_count LOOP
      INSERT INTO challenge_stats (
        ...
        floor(random() * 10 + 1)::integer,  -- 1-11 assists
        floor(random() * 5000 + 1000)::integer, -- 1000-6000 score
        i + 1, -- placement 2, 3, 4, etc.
        true
      );
    END LOOP;
    
    -- 8. Process payouts using new safe payout function
    ...
    -- Get winner information (first place)
    ...
    RETURN jsonb_build_object(
      ...
      'challenge_id', challenge_id,
      'crashed', false,
      'participants', participants_count + 1,  -- Include creator
      'total_pot', total_pot,
