...
CREATE OR REPLACE FUNCTION settle_challenge_payouts(challenge_id UUID) 
RETURNS VOID 
LANGUAGE plpgsql 
SECURITY DEFINER 
SET search_path = 'public'
...
CREATE OR REPLACE FUNCTION refund_all_challenge_players(
  challenge_id UUID,
  reason TEXT
) RETURNS TABLE(user_id UUID, refund_amount NUMERIC) 
LANGUAGE plpgsql 
SECURITY DEFINER 
SET search_path = 'public'
AS $$
...
ALTER TABLE market_engine_errors ENABLE ROW LEVEL SECURITY;

-- 3. Create RLS policies for new tables
CREATE POLICY "Admins can view market events" ON market_events
  FOR SELECT USING (is_user_admin());

CREATE POLICY "Service can insert market events" ON market_events
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Admins can view market engine errors" ON market_engine_errors
  FOR SELECT USING (is_user_admin());

CREATE POLICY "Service can insert market engine errors" ON market_engine_errors
  FOR INSERT WITH CHECK (true);