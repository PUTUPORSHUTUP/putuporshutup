...
-- inserts/updates by automation happen with service_role (bypass RLS)
...
-- insert/update done by service_role only; no user writes
...
drop policy if exists mq_update_delete_own_waiting on market_queue;
create policy mq_update_delete_own_waiting on market_queue
...
-- ===============================
-- otp_verifications (sensitive)
-- RECOMMENDED: service_role only; deny users
-- ===============================
...
-- allow inserts from both anon & authenticated:
--  - authenticated users must write their own user_id
--  - anon writes must set user_id NULL
...
-- prevent normal users from updating/deleting old events
...
-- ===============================
-- Helpful: grant basic SELECT to service_role (bypasses RLS anyway)
-- ===============================
