...
-- Keep existing secure policies but ensure they're properly restrictive
-- (The existing policies like "Players can view their match results" are already secure)

-- =============================================================================
-- TRANSACTIONS TABLE - Fix Financial Transaction Exposure
-- =============================================================================

-- Check if transactions table has overly permissive policies and fix them
DO $$
BEGIN
  -- Drop any overly permissive policies on transactions
  EXECUTE 'DROP POLICY IF EXISTS "transactions_read_all" ON public.transactions';
  EXECUTE 'DROP POLICY IF EXISTS "Users can view all transactions" ON public.transactions';
