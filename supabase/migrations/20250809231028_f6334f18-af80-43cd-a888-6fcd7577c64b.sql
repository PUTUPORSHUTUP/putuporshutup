...
begin
  select winner, loser into r from public.fraud_match_results where id = p_res;
  select count(*) into c
  from public.fraud_match_results
...
begin
  ...
  from public.fraud_match_results where id = p_res;
...
begin
  select screenshot_hash, winner into h, uid from public.fraud_match_results where id = p_res;
...
begin
  select winner, loser into a, b from public.fraud_match_results where id = p_res;
...
create or replace function public.after_fraud_match_result_checks()
...
drop trigger if exists trg_after_fraud_match_result on public.fraud_match_results;
create trigger trg_after_fraud_match_result
after insert on public.fraud_match_results
for each row execute function public.after_fraud_match_result_checks();