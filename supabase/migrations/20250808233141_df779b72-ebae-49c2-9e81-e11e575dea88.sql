...
BEGIN
  ...
  -- PHASE 5: PROCESS PAYOUTS - Top 3 get prizes
  WITH prizes AS (
    SELECT 
      mr.player_id,
      CASE mr.placement
        WHEN 1 THEN 400::NUMERIC
        WHEN 2 THEN 240::NUMERIC
        WHEN 3 THEN 160::NUMERIC
        ELSE 0::NUMERIC
      END AS amount
  ...
  WHERE mr.match_id = challenge_id
  AND mr.placement <= 3
  AND CASE mr.placement WHEN 1 THEN 400 WHEN 2 THEN 240 WHEN 3 THEN 160 ELSE 0 END > 0;
...
INSERT INTO profiles (user_id, username, display_name, wallet_balance, is_test_user)
SELECT 
  gen_random_uuid(),
  'test_user_' || generate_series(1, 20),
  'Test Player ' || generate_series(1, 20),
  1000,
  TRUE
